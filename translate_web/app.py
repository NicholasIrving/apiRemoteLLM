...
def create_app(args):
    ...
    @bp.errorhandler(InfoError)
    def infoerror(e):
        return jsonify({"error": "Check the Prompt, Model and Temperature fields!"}), e.code
    ...
    def translate():
        ...
        try:
            if source_lang == "auto":
                source_lang = detected_src_lang['language']
            ...
            for lang in __:
                if lang.code == source_lang:
                    source_language_name = lang.name
                    break
            for lang in __:
            ...
            if batch:
                ...
                batch_alternatives = []
                for text in q:
                    translated_text = api_remote.apps.translate(q, source_language_name, target_language_name, prompt, model, temperature).text
                    response = json_.loads(translated_text)
                ...
                # result = {"translatedText": "调试中"}
                translated_text = api_remote.apps.translate(q, source_language_name, target_language_name, prompt, model, temperature).text
                response = json_.loads(translated_text)
                result = {"translatedText": response["response"]}
