...
from .default_values import DEFAULT_ARGUMENTS as DEFARGS
...
def get_args(**kwargs):
    ...
    parser.add_argument(
        "--host", type=str, help="Hostname (%(default)s)", default=kwargs.get('host') or DEFARGS['HOST']
    )
    parser.add_argument("--port", type=int, help="Port (%(default)s)", default=kwargs.get('port') or DEFARGS['PORT'])
    parser.add_argument(
        "--char-limit",
        default=kwargs.get('char-limit') or DEFARGS['CHAR_LIMIT'],
        type=int,
    ...
    parser.add_argument(
        "--req-limit",
        default=kwargs.get('req-limit') or DEFARGS['REQ_LIMIT'],
        type=int,
        metavar="<number>",
    ...
    parser.add_argument(
        "--req-limit-storage",
        default=kwargs.get('req-limit-storage') or DEFARGS['REQ_LIMIT_STORAGE'],
        type=str,
        metavar="<Storage URI>",
    ...
    parser.add_argument(
        "--hourly-req-limit",
        default=kwargs.get('hourly-req-limit') or DEFARGS['HOURLY_REQ_LIMIT'],
        type=int,
        metavar="<number>",
    ...
    parser.add_argument(
        ...
        default=kwargs.get('hourly-req-limit-decay') or DEFARGS['HOURLY_REQ_LIMIT_DECAY'],
        type=int,
        metavar="<number>",
    ...
    parser.add_argument(
        "--daily-req-limit",
        default=kwargs.get('daily-req-limit') or DEFARGS['DAILY_REQ_LIMIT'],
        type=int,
        metavar="<number>",
    ...
    parser.add_argument(
        "--req-flood-threshold",
        default=kwargs.get('req-flood-threshold') or DEFARGS['REQ_FLOOD_THRESHOLD'],
        type=int,
        metavar="<number>",
    ...
    parser.add_argument(
        "--req-time-cost",
        default=kwargs.get('req-time-cost') or DEFARGS['REQ_TIME_COST'],
        type=int,
        metavar="<number>",
    ...
    parser.add_argument(
        "--batch-limit",
        default=kwargs.get('batch-limit') or DEFARGS['BATCH_LIMIT'],
        type=int,
    ...
    parser.add_argument(
        "--ga-id",
        type=str,
        default=kwargs.get('ga-id') or DEFARGS['GA_ID'],
        metavar="<GA ID>",
        help="Enable Google Analytics on the API client page by providing an ID (%(default)s)",
    )
    parser.add_argument(
        "--debug", default=kwargs.get('debug') or DEFARGS['DEBUG'], action="store_true", help="Enable debug environment"
    )
    parser.add_argument(
        "--ssl", default=kwargs.get('ssl') or DEFARGS['SSL'], action="store_true", help="Whether to enable SSL"
    )
    parser.add_argument(
        "--frontend-language-source",
        type=str,
        default=kwargs.get('frontend-language-source') or DEFARGS['FRONTEND_LANGUAGE_SOURCE'],
    ...
    parser.add_argument(
        ...
        type=str,
        default=kwargs.get('frontend-language-target') or DEFARGS['FRONTEND_LANGUAGE_TARGET'],
        metavar="<language code>",
    ...
    parser.add_argument(
        ...
        type=str,
        default=kwargs.get('require-api-key-origin') or DEFARGS['REQUIRE_API_KEY_ORIGIN'],
    ...
    parser.add_argument(
        ...
        default=kwargs.get('require-api-key-secret') or DEFARGS['REQUIRE_API_KEY_SECRET'],
        action="store_true",
    ...
    parser.add_argument(
        "--shared-storage",
        type=str,
        default=kwargs.get('shared-storage') or DEFARGS['SHARED_STORAGE'],
        metavar="<Storage URI>",
        help="Shared storage URI to use for multi-process data sharing (e.g. via gunicorn)",
    )
    parser.add_argument(
        "--secondary",
        default=kwargs.get('secondary') or DEFARGS['SECONDARY'],
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=kwargs.get('load-only') or DEFARGS['LOAD_ONLY'],
        metavar="<comma-separated language codes>",
        help="Set available languages (ar,de,en,es,fr,ga,hi,it,ja,ko,pt,ru,zh)",
    )
    parser.add_argument(
        "--alternatives-limit",
        default=kwargs.get('alternatives-limit') or DEFARGS['ALTERNATIVES_LIMIT'],
        type=int,
    ...
    parser.add_argument(
        "--threads",
        default=kwargs.get('threads') or DEFARGS['THREADS'],
        type=int,
        metavar="<number of threads>",
        help="Set number of threads (%(default)s)",
    )
    parser.add_argument(
        "--suggestions", default=kwargs.get('suggestions') or DEFARGS['SUGGESTIONS'], action="store_true", help="Allow user suggestions"
    )
    parser.add_argument(
        "--disable-files-translation", default=kwargs.get('disable-files-translation') or DEFARGS['DISABLE_FILES_TRANSLATION'], action="store_true",
        help="Disable files translation"
    )
    parser.add_argument(
        "--disable-web-ui", default=kwargs.get('disable-web-ui') or DEFARGS['DISABLE_WEB_UI'], action="store_true", help="Disable web ui"
    )
    parser.add_argument(
        "--update-models", default=kwargs.get('update-models') or DEFARGS['UPDATE_MODELS'], action="store_true", help="Update language models at startup"
    )
    parser.add_argument(
        "--force-update-models", default=kwargs.get('force-update-models') or DEFARGS['FORCE_UPDATE_MODELS'], action="store_true", help="Install/Reinstall language models at startup"
    )
    parser.add_argument(
        "--metrics",
        default=kwargs.get('metrics') or DEFARGS['METRICS'],
        action="store_true",
        help="Enable the /metrics endpoint for exporting Prometheus usage metrics",
    )
    parser.add_argument(
        "--metrics-auth-token",
        default=kwargs.get('metrics-auth-token') or DEFARGS['METRICS_AUTH_TOKEN'],
        type=str,
        help="Protect the /metrics endpoint by allowing only clients that have a valid Authorization Bearer token (%(default)s)",
    )
    parser.add_argument(
        "--url-prefix",
        default=kwargs.get('url-prefix') or DEFARGS['URL_PREFIX'],
        type=str,
